...
int ReadSourceFromFile(const char* fileName, char** source, size_t* sourceSize)
{
    ...
    if (fp == NULL)
    {
        LogError("Couldn't find program source file '%s'.", fileName);
        ...
        *source = new char[*sourceSize];
        if (*source == NULL)
        {
            LogError("Couldn't allocate %d bytes for program source from file '%s'.", *sourceSize, fileName);
...
int OCLKernel::CreateAndBuildKernel(std::string openCLFileName, std::string kernelName, std::string buildOptions)
{
    cl_int error_code;
    size_t src_size = 0;

    // Obtaining the OpenCL context from the command-queue properties
    ...
    if (CL_SUCCESS != error_code)
    {
        LogError("clGetCommandQueueInfo (CL_QUEUE_DEVICE) returned %s.", TranslateOpenCLError(error_code));
        goto Finish;
    }
    ...
    if (CL_SUCCESS != error_code)
    {
        LogError("clGetDeviceInfo (CL_DEVICE_LOCAL_MEM_SIZE) returned %s.", TranslateOpenCLError(error_code));
        goto Finish;
    }
    ...
    if (CL_SUCCESS != error_code)
    {
        LogError("ReadSourceFromFile returned %s.", TranslateOpenCLError(error_code));
        goto Finish;
    }
    ...
    if (CL_SUCCESS != error_code)
    {
        LogError("clCreateProgramWithSource returned %s.", TranslateOpenCLError(error_code));
        goto Finish;
    }
    ...
    {
        LogError("clBuildProgram() for source program returned %s.", TranslateOpenCLError(error_code));
        ...
        printf("Build Fail Log: \t%s", build_log);
    ...
    if (CL_SUCCESS != error_code)
    {
        LogError("clCreateKernel returned %s.", TranslateOpenCLError(error_code));
        goto Finish;
    }
Finish:
    if (source)
...
tDeviceRC OCLKernel::enqueue(int dimension, size_t global_work_offset[3], size_t global_work_size[3], size_t local_work_size[3]) {
    ...
    if (CL_SUCCESS != error_code)
    {
        LogError("clEnqueueNDRangeKernel returned %s.", TranslateOpenCLError(error_code));
        return error_code;
    }
    return CL_SUCCESS;
}
